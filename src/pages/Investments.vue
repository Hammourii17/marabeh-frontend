      ...
      >
        ...
        <div class="flex-1 flex gap-2 overflow-x-auto">
          ...
          <div class="data-item">
            ...
            <span class="data-value">{{ inv.collateral_coverage ?? 0 }}%</span>
          </div>
          <div class="data-item">
...
<script setup>
  ...
  const data = computed(() => mainStore.investments.results)
  const page = ref(Number(route.query.page) || 1)
  const pageCount = computed(() => Math.ceil(mainStore.investments.count / 8))

  onMounted(async () => {
    await mainStore.fetchInvestments(page.value)
  })

  const updatePage = (currentPage) => {
    page.value = currentPage
    mainStore.fetchInvestments(page.value)
